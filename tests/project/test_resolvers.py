...
class TestResolvers(TestCase):
    def setUp(self):
        ...
        self.comments = comment_model.objects.all().order_by('created_at', 'pk')
        self.paginate_by = 7
    ...
    def _get_paginator(self, queryset, position_page: int):
        ...
        # Check is it `position_page`, which was taken from utility,
        # really exists in paginated object_list in specific page
    ...
    def test_models_resolver_page_from_nested_object(self):
        ...
        comments = CommentFactory.create_batch(size=75, post=post, with_delay=True)
        ...
        paginator, paginated_page = self._get_paginator(post.comments.all().order_by('created_at', 'pk'), position_page)
    ...
    def test_num_queries_page_from_nested_object(self):
        post = PostFactory.create()
        comments = CommentFactory.create_batch(size=75, post=post, with_delay=True)
        target_comment = comments[random.randint(0, 74)]
        self.assertNumQueries(
            1, post.get_page_from_nested_object, target_child_instance=target_comment, items_per_page=self.paginate_by
        )

    def test_num_queries_page_from_queryset(self):
        comments = self.comments
        target_comment = comments[self.random_range_number]

        self.assertNumQueries(
            # 1st query - taking all comments for test purposes via .all() (which is logically),
            # 2nd query - select count(*) from qs.filter(**{field_name + lookup: getattr(target, field_name)}).count()
            2,
            target_comment.get_page_from_queryset,
            queryset=comments,
